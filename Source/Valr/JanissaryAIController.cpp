...
#include "JanissaryAIController.h"
#include <ctime>
#include <string>

int AJanissaryAIController::BiasRand(const int arr[], int size)
{
	int random = FMath::RandRange(0, size);
	return arr[random];
}

TArray<double> AJanissaryAIController::SoftMax(TArray<double> values)
{
...
void AJanissaryAIController::BeginPlay()
{
	Super::BeginPlay();
	Fighter = Cast<AJanissary>(GetOwner());
	...
	{
	case DIFFICULTY::EASY: difficultyTimer = 35; difficultyBlockChance = 20; path = TCHAR_TO_UTF8(*(FPaths::ProjectContentDir() + "model_Janissary_EASY.wdt")); break;
	case DIFFICULTY::NORMAL: difficultyTimer = 25; difficultyBlockChance = 35; path = TCHAR_TO_UTF8(*(FPaths::ProjectContentDir() + "model_Janissary_NORMAL.wdt")); break;
	case DIFFICULTY::HARD: difficultyTimer = 15; difficultyBlockChance = 50; path = TCHAR_TO_UTF8(*(FPaths::ProjectContentDir() + "model_Janissary_HARD.wdt")); break;
	}
	Network->LoadWeights(path.c_str());
}

void AJanissaryAIController::Tick(float DeltaTime)
{
	...
	if (bTrain)
	{
		...
		else
		{
			...
			{
				if (FMath::RandRange(0, 100) < 30 && Fighter->specialMeter >= 100)
				{
					if (Timer == 0)
					{
						maxQIndex = 11;
	...
	if (bTrain)
	{
		...
		if (exploreRate <= minExploreRate + 1 && time.GetSecond() == 0)
		{
			switch (Manager->Instance->Difficulty)
			{
			case DIFFICULTY::EASY: Network->SaveWeights("model_Janissary_EASY"); break;
			case DIFFICULTY::NORMAL: Network->SaveWeights("model_Janissary_NORMAL"); break;
			case DIFFICULTY::HARD: Network->SaveWeights("model_Janissary_HARD"); break;
			}
		}
	}
}
