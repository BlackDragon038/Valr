...
void ANinjaAIController::BeginPlay()
{
	...
	{
	case DIFFICULTY::EASY: difficultyTimer = 35; difficultyBlockChance = 20; path = TCHAR_TO_UTF8(*(FPaths::ProjectContentDir() + "model_Ninja_EASY.wdt")); break;
	case DIFFICULTY::NORMAL: difficultyTimer = 25; difficultyBlockChance = 35; path = TCHAR_TO_UTF8(*(FPaths::ProjectContentDir() + "model_Ninja_NORMAL.wdt")); break;
	case DIFFICULTY::HARD: difficultyTimer = 15; difficultyBlockChance = 50; path = TCHAR_TO_UTF8(*(FPaths::ProjectContentDir() + "model_Ninja_HARD.wdt")); break;
	}
...
void ANinjaAIController::Tick(float DeltaTime)
{
	...
	if (bTrain)
	{
		...
		if (exploreRate <= minExploreRate + 1 && time.GetSecond() == 0)
		{
			switch (Manager->Instance->Difficulty)
			{
			case DIFFICULTY::EASY: Network->SaveWeights("model_Ninja_EASY"); break;
			case DIFFICULTY::NORMAL: Network->SaveWeights("model_Ninja_NORMAL"); break;
			case DIFFICULTY::HARD: Network->SaveWeights("model_Ninja_HARD"); break;
			}
		}
	}
}
