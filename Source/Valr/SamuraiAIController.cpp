...
#include "SamuraiAIController.h"
#include <ctime>
#include <string>

int ASamuraiAIController::BiasRand(const int arr[], int size)
{
	int random = FMath::RandRange(0, size);
	return arr[random];
}

TArray<double> ASamuraiAIController::SoftMax(TArray<double> values)
{
...
void ASamuraiAIController::BeginPlay()
{
	Super::BeginPlay();
	Fighter = Cast<ASamurai>(GetOwner());
	...
	{
	case DIFFICULTY::EASY: difficultyTimer = 35; difficultyBlockChance = 20; path = TCHAR_TO_UTF8(*(FPaths::ProjectContentDir() + "model_Samurai_EASY.wdt")); break;
	case DIFFICULTY::NORMAL: difficultyTimer = 25; difficultyBlockChance = 35; path = TCHAR_TO_UTF8(*(FPaths::ProjectContentDir() + "model_Samurai_NORMAL.wdt")); break;
	case DIFFICULTY::HARD: difficultyTimer = 15; difficultyBlockChance = 50; path = TCHAR_TO_UTF8(*(FPaths::ProjectContentDir() + "model_Samurai_HARD.wdt")); break;
	}
	Network->LoadWeights(path.c_str());
}

void ASamuraiAIController::Tick(float DeltaTime)
{
	...
	if (bTrain)
	{
		...
		if (exploreRate <= minExploreRate + 1 && time.GetSecond() == 0)
		{
			switch (Manager->Instance->Difficulty)
			{
			case DIFFICULTY::EASY: Network->SaveWeights("model_Samurai_EASY"); break;
			case DIFFICULTY::NORMAL: Network->SaveWeights("model_Samurai_NORMAL"); break;
			case DIFFICULTY::HARD: Network->SaveWeights("model_Samurai_HARD"); break;
			}
		}
	}
}
