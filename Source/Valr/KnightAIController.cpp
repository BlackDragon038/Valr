...
int AKnightAIController::BiasRand(const int arr[], int size)
{
	int random = FMath::RandRange(0, size);
	return arr[random];
}

TArray<double> AKnightAIController::SoftMax(TArray<double> values)
{
...
void AKnightAIController::Tick(float DeltaTime)
{
	...
	if (bTrain)
	{
		...
		if (exploreRate <= minExploreRate + 1 && time.GetSecond() == 0)
		{
			switch (Manager->Instance->Difficulty)
			{
			case DIFFICULTY::EASY: Network->SaveWeights("model_Knight_EASY"); break;
			case DIFFICULTY::NORMAL: Network->SaveWeights("model_Knight_NORMAL"); break;
			case DIFFICULTY::HARD: Network->SaveWeights("model_Knight_HARD"); break;
			}
		}
	}
}
