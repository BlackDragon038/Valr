...
#include "CrusaderAIController.h"
#include <ctime>
#include <string>
...
int ACrusaderAIController::BiasRand(const int arr[], int size)
{
	int random = FMath::RandRange(0,size);
	return arr[random];
}

void ACrusaderAIController::BeginPlay()
{
	Super::BeginPlay();
	Fighter = Cast<ACrusader>(GetOwner());
	...
	{
		case DIFFICULTY::EASY: difficultyTimer = 35; difficultyBlockChance = 20; path = TCHAR_TO_UTF8(*(FPaths::ProjectContentDir() + "model_Crusader_EASY.wdt")); break;
		case DIFFICULTY::NORMAL: difficultyTimer = 25; difficultyBlockChance = 35; path = TCHAR_TO_UTF8(*(FPaths::ProjectContentDir() + "model_Crusader_NORMAL.wdt")); break;
		case DIFFICULTY::HARD: difficultyTimer = 15; difficultyBlockChance = 50; path = TCHAR_TO_UTF8(*(FPaths::ProjectContentDir() + "model_Crusader_HARD.wdt")); break;
	}
	Network->LoadWeights(path.c_str());
}

void ACrusaderAIController::Tick(float DeltaTime)
{
	...
	if (bTrain)
	{
		...
		if (FVector::Distance(Fighter->GetActorLocation(), Manager->Player1->GetActorLocation()) > averageRange * 1.15f || 
			AFightManager::Angle(Fighter->GetActorForwardVector(), Dir) > 70)
		{
	...
	if (maxQIndex != 11 && Fighter->attackType == ATTACK_TYPE::SPECIAL) Fighter->ReleasedSpecial();
	...
	{
		FDateTime time;
		if (exploreRate <= minExploreRate && time.GetSecond() == 0)
		{
			switch (Manager->Instance->Difficulty)
			{
