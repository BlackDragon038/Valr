...
#include "VikingAIController.h"
#include <ctime>
#include <string>

int AVikingAIController::BiasRand(const int arr[], int size)
{
	int random = FMath::RandRange(0, size);
	return arr[random];
}

TArray<double> AVikingAIController::SoftMax(TArray<double> values)
{
...
void AVikingAIController::BeginPlay()
{
	Super::BeginPlay();
	...
	{
	case DIFFICULTY::EASY: difficultyTimer = 35; difficultyBlockChance = 20; path = TCHAR_TO_UTF8(*(FPaths::ProjectContentDir() + "model_Viking_EASY.wdt")); break;
	case DIFFICULTY::NORMAL: difficultyTimer = 25; difficultyBlockChance = 35; path = TCHAR_TO_UTF8(*(FPaths::ProjectContentDir() + "model_Viking_NORMAL.wdt")); break;
	case DIFFICULTY::HARD: difficultyTimer = 15; difficultyBlockChance = 50; path = TCHAR_TO_UTF8(*(FPaths::ProjectContentDir() + "model_Viking_HARD.wdt")); break;
	}
...
void AVikingAIController::Tick(float DeltaTime)
{
			...
			else
			{
				...
				else
				{
					if (FMath::RandRange(0, 100) < exploreRate && Timer == 0)
					{
						...
						switch (Manager->Instance->Difficulty)
						{
						case DIFFICULTY::EASY: attackChance = 50; break;
						case DIFFICULTY::NORMAL: attackChance = 70; break;
	...
	if (bTrain)
	{
		...
		if (exploreRate <= minExploreRate + 1 && time.GetSecond() == 0)
		{
			switch (Manager->Instance->Difficulty)
			{
			case DIFFICULTY::EASY: Network->SaveWeights("model_Viking_EASY"); break;
			case DIFFICULTY::NORMAL: Network->SaveWeights("model_Viking_NORMAL"); break;
			case DIFFICULTY::HARD: Network->SaveWeights("model_Viking_HARD"); break;
			}
		}
	}
}